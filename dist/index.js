...
const argv = yargs(hideBin(process.argv))
    ...
    .parseSync();
// Параметры
const openRouterApiKey = argv.apiKey;
const openRouterApiUrl = "https://openrouter.ai/api/v1/chat/completions";
const defaultModel = argv.defaultModel;
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                language: {
                    type: "string",
                    description: "Primary language of the project (e.g., js, python, csharp, php, java, ruby, go, rust, cpp, swift)",
                },
            },
...
class OpenRouterServer {
    ...
    // Поиск корня проекта
    getProjectRoot(projectPath, language) {
    ...
    async callTool(params) {
        ...
        try {
            if (name === "ask") {
                const { prompt, model = defaultModel } = args;
                if (!prompt) {
            ...
            if (name === "code_review") {
                const { completed_tasks = [], work_report = "", project_path = "", planned_tasks = [], language = "", model = defaultModel, } = args;
